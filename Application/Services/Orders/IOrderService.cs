...
namespace Application.Services.Orders
{
    ...
    public class OrderService : IOrderService
    {
        ...
        public OrderService(IDataBaseContext context
            ,IUriComposerService uriComposer
            ,IMapper mapper
        ...
        public int CreateOrder(int BasketId, int UserAddressId, PaymentMethod paymentMethod)
        {
            ...
            var orderItems = basket.Items.Select(basketItems =>
              {
                  var catalogItem = catalogItems.First(c => c.Id == basketItems.CatalogItemId);
                  var orderItem = new OrderItem(catalogItem.Id
                      , catalogItem.Name
                      , uriComposer
                      .ComposeImageUri(catalogItem?.CatalogItemImages?.FirstOrDefault()?.Src ?? "")
                      , catalogItem.Price
